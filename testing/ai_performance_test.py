...
if __name__ == '__main__':
    ...
    parser.add_argument("--games", type=int, help="Number of games to run (overrides config)")
    parser.add_argument("--speed", type=int, help="Game speed (overrides config)")
    parser.add_argument("--grid", type=int, nargs=2, help="Grid size (overrides config)")
    parser.add_argument("--save", action="store_true", help="Save detailed results to file")
    parser.add_argument('--config', type=str, default='config.yaml', help='Path to YAML config file')
    args = parser.parse_args()
    config = load_config(args.config)
    num_games = args.games if args.games is not None else config['testing'].get('games', 50)
    speed = args.speed if args.speed is not None else config['testing'].get('speed', 60)
    grid = tuple(args.grid) if args.grid is not None else tuple(config['testing'].get('grid_size', [20, 20]))
